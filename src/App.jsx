      ...
      <aside className={`fixed left-0 top-0 h-full bg-dark-purple-900/90 backdrop-blur-sm border-r border-dark-purple-700 z-40 transition-all duration-300 ${sidebarOpen ? 'w-72' : 'w-0'} overflow-hidden`}>
        <div className="p-4 h-full overflow-y-auto">
          ...
          <div className="space-y-3 text-xs">
            <div className="bg-dark-purple-950 p-2 rounded border border-dark-purple-700">
              ...
              {character.level < 20 && (
                <button
                  ...
                  className="mt-1 text-[10px] text-dark-purple-400 hover:text-white"
                >
                  + Roll HP
                </button>
              )}
            ...
            <div className="bg-dark-purple-950 p-2 rounded border border-dark-purple-700">
              <div className="text-dark-purple-400">AC</div>
              <div className="text-xl font-bold text-white">{character.armorClass || 10}</div>
            </div>

            <div className="bg-dark-purple-950 p-2 rounded border border-dark-purple-700">
              <div className="text-dark-purple-400">Initiative</div>
              <div className="text-xl font-bold text-white">{character.initiative >= 0 ? '+' : ''}{character.initiative || 0}</div>
              <button onClick={() => handleRoll(rollInitiative, 'Initiative')} className="text-[10px] text-dark-purple-400 hover:text-white">🎲 Roll</button>
            </div>

            <div className="bg-dark-purple-950 p-2 rounded border border-dark-purple-700">
              <div className="text-dark-purple-400">Speed</div>
              <div className="text-xl font-bold text-white">{character.speed || 30}ft</div>
            </div>

            <div className="bg-dark-purple-950 p-2 rounded border border-dark-purple-700">
              <div className="text-dark-purple-400">Proficiency</div>
              <div className="text-xl font-bold text-white">+{character.proficiencyBonus || 2}</div>
            </div>
      ...
      {showRollHistory && (
        <div className="fixed right-4 top-4 w-64 bg-dark-purple-900/95 backdrop-blur-sm border border-dark-purple-700 rounded-lg p-3 z-50 max-h-96 overflow-y-auto shadow-xl">
          ...
          {rollHistory?.length === 0 ? (
            <p className="text-dark-purple-400 text-xs">No rolls yet</p>
          ) : (
      ...
      {lastRoll && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 bg-dark-purple-900/95 backdrop-blur-sm border border-dark-purple-700 rounded-lg p-3 z-50 shadow-xl">
          <div className="flex justify-between items-center gap-4">
            <div>
              <span className="text-dark-purple-400 text-xs">{lastRoll.label}</span>
              <div className="text-xl font-bold text-white">{lastRoll.result?.total || lastRoll.result?.gain || lastRoll.result}</div>
            </div>
        ...
        <header className="bg-dark-purple-900 border-b border-dark-purple-700 sticky top-0 z-30">
          <div className="max-w-5xl mx-auto px-4 py-3">
            <div className="flex items-center justify-between mb-3">
              <button
                onClick={() => setSidebarOpen(!sidebarOpen)}
                className="bg-dark-purple-800 hover:bg-dark-purple-700 text-white font-semibold py-1.5 px-3 rounded-lg transition-all duration-200 border border-dark-purple-600 text-sm"
              >
            ...
            {showStatOptions && (
              <div className="bg-dark-purple-950 border border-dark-purple-700 rounded-lg p-3 mb-3">
                <h3 className="text-sm font-bold text-white mb-2">Ability Score Generation</h3>
                <div className="grid grid-cols-3 gap-2">
                  <button
                    onClick={() => setStatMethod('standard')}
                    className={`p-2 rounded border transition-all text-xs ${character.statMethod === 'standard' ? 'bg-dark-purple-600 border-white text-white' : 'bg-dark-purple-900 border-dark-purple-700 text-dark-purple-300 hover:border-dark-purple-500'}`}
                  >
                    <div className="font-bold">Standard</div>
                    <div className="opacity-80">15,14,13,12,10,8</div>
                  </button>
                  <button
                    onClick={() => setStatMethod('pointbuy')}
                    className={`p-2 rounded border transition-all text-xs ${character.statMethod === 'pointbuy' ? 'bg-dark-purple-600 border-white text-white' : 'bg-dark-purple-900 border-dark-purple-700 text-dark-purple-300 hover:border-dark-purple-500'}`}
                  >
                    <div className="font-bold">Point Buy</div>
                    <div className="opacity-80">27 points</div>
                  </button>
                  <button
                    onClick={() => setStatMethod('roll')}
                    className={`p-2 rounded border transition-all text-xs ${character.statMethod === 'roll' ? 'bg-dark-purple-600 border-white text-white' : 'bg-dark-purple-900 border-dark-purple-700 text-dark-purple-300 hover:border-dark-purple-500'}`}
                  >
                    <div className="font-bold">Roll 4d6</div>
                    <div className="opacity-80">Drop lowest</div>
                  </button>
                </div>
                <button
                  onClick={() => handleRoll(rollAbilityScores, 'Ability Scores')}
                  className="mt-2 w-full bg-dark-purple-600 hover:bg-dark-purple-500 text-white font-bold py-2 px-4 rounded-lg transition-all duration-200 text-sm"
                >
                  🎲 Roll New Stats
                </button>
              </div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-1.5">
              {tabs.map((tab, index) => (
                <button
                  ...
                  className={`py-2 px-1 rounded-lg font-semibold transition-all duration-200 text-[10px] md:text-xs border ${activeTab === index ? `${tab.color} text-white font-bold shadow-lg border-white/30` : 'bg-dark-purple-900/50 text-dark-purple-300 hover:bg-dark-purple-800 hover:text-white border-dark-purple-700'}`}
                  title={tab.friend}
                >
            ...
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="block text-xs font-medium mb-1.5 text-dark-purple-300">Age: <span className="text-dark-purple-400">{ageValue} yrs</span></label>
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1.5 text-dark-purple-300">Height: <span className="text-dark-purple-400">{inchesToFeetInches(heightValue)}</span></label>
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1.5 text-dark-purple-300">Weight: <span className="text-dark-purple-400">{weightValue} lbs</span></label>
                    ...
                    <div>
                      <label className="block text-xs font-medium mb-1.5 text-dark-purple-300">Gender</label>
                      <input type="text" value={character.gender || ''} onChange={(e) => updateCharacter('gender', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-dark-purple-500 transition-all" placeholder="e.g., Male, Female" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-6 text-center">
                ...
                <button onClick={() => {
                  ...
                  link.click()
                }} className="bg-dark-purple-600 hover:bg-dark-purple-500 text-white font-bold py-3 px-6 rounded-lg transition-all duration-200 shadow-lg">
                  📥 Download
                </button>
              </div>
            </div>
              ...
              <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-6 text-center">
                ...
                <div className="grid md:grid-cols-2 gap-3 max-w-md mx-auto">
                  {['Race','Class','Background','Item'].map(item => (
                    <button key={item} className="bg-dark-purple-800 hover:bg-dark-purple-700 text-white font-semibold py-2 px-4 rounded-lg transition-all border border-dark-purple-600 text-sm">
                      Create {item}
                    </button>
                  ))}
                </div>
