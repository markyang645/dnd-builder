...
import { abilityLabels, classHitDice } from './data/dndRules'
    ...
    return (
      <div className="min-h-screen bg-friend-gradient flex items-center justify-center p-4">
        <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-8 max-w-md w-full text-center shadow-xl">
          ...
          <p className="text-dark-purple-400 text-sm">Uses Standard Array (15, 14, 13, 12, 10, 8)</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowRollHistory(!showRollHistory)} className="bg-dark-purple-800 hover:bg-dark-purple-700 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-200 border border-dark-purple-600" title="View Roll History">
                📜 {rollHistory?.length || 0}
              </button>
              <button onClick={() => setShowStatOptions(!showStatOptions)} className="bg-dark-purple-800 hover:bg-dark-purple-700 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-200 border border-dark-purple-600">
                📊 Stats
              </button>
              <button onClick={() => createCharacter('New Character', 'standard')} className="bg-dark-purple-800 hover:bg-dark-purple-700 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-200 border border-dark-purple-600">
                + New
              </button>
            </div>
          </div>
          ...
          {showStatOptions && (
            <div className="bg-dark-purple-950 border border-dark-purple-700 rounded-lg p-4 mb-4">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <button onClick={() => setStatMethod('standard')} className={`p-4 rounded-lg border-2 transition-all ${character.statMethod === 'standard' ? 'bg-dark-purple-600 border-white text-white' : 'bg-dark-purple-900 border-dark-purple-700 text-dark-purple-300 hover:border-dark-purple-500'}`}>
                  <div className="font-bold">Standard Array</div>
                  <div className="text-sm opacity-80">15, 14, 13, 12, 10, 8</div>
                </button>
                <button onClick={() => setStatMethod('pointbuy')} className={`p-4 rounded-lg border-2 transition-all ${character.statMethod === 'pointbuy' ? 'bg-dark-purple-600 border-white text-white' : 'bg-dark-purple-900 border-dark-purple-700 text-dark-purple-300 hover:border-dark-purple-500'}`}>
                  <div className="font-bold">Point Buy</div>
                  <div className="text-sm opacity-80">27 points to spend</div>
                </button>
                <button onClick={() => setStatMethod('roll')} className={`p-4 rounded-lg border-2 transition-all ${character.statMethod === 'roll' ? 'bg-dark-purple-600 border-white text-white' : 'bg-dark-purple-900 border-dark-purple-700 text-dark-purple-300 hover:border-dark-purple-500'}`}>
                  <div className="font-bold">Roll 4d6</div>
                  <div className="text-sm opacity-80">Drop lowest die</div>
                </button>
              </div>
              <button onClick={() => handleRoll(rollAbilityScores, 'Ability Scores')} className="mt-4 w-full bg-dark-purple-600 hover:bg-dark-purple-500 text-white font-bold py-3 px-6 rounded-lg transition-all duration-200">
          ...
          {showRollHistory && (
            <div className="bg-dark-purple-950 border border-dark-purple-700 rounded-lg p-4 mb-4 max-h-64 overflow-y-auto">
              <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={clearRollHistory} className="text-dark-purple-400 hover:text-white text-sm">Clear All</button>
              </div>
              {rollHistory?.length === 0 ? (
            ...
            <div className="bg-dark-purple-950 border border-dark-purple-700 rounded-lg p-4 mb-4">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <div className="text-2xl font-bold text-white">{lastRoll.result?.total || lastRoll.result?.gain || lastRoll.result}</div>
                </div>
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
            {tabs.map((tab, index) => (
              <button key={tab.name} onClick={() => setActiveTab(index)} className={`py-3 px-2 rounded-lg font-semibold transition-all duration-200 text-xs md:text-sm border-2 ${activeTab === index ? `${tab.color} text-white font-bold shadow-lg border-white/30` : 'bg-dark-purple-900/50 text-dark-purple-300 hover:bg-dark-purple-800 hover:text-white border-dark-purple-700'}`} title={tab.friend}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
            ...
            <div className="bg-dark-purple-950 p-4 rounded-lg border border-dark-purple-700 text-center">
              ...
              {character.hpHistory?.length > 0 && (
                <div className="mt-2 text-xs text-dark-purple-400">
                  <div className="font-semibold text-dark-purple-300 mb-1">HP by Level:</div>
                  <div className="max-h-24 overflow-y-auto space-y-1 text-left">
                    {character.hpHistory.map((hp, i) => (
                      <div key={i} className="flex justify-between border-b border-dark-purple-800 pb-1">
                        <span>Lv.{hp.level}</span>
                        <span>+{hp.gain} ({hp.roll})</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <button
                onClick={() => {
                  if (character.level < 20) {
                    const result = handleRoll(rollLevelUpHP, `Level ${character.level + 1} HP`)
                    if (result && !result.error) {
                      alert(`🎉 Level Up!\n\n${result.result}\n\n+${result.gain} HP\nNew Total: ${character.maxHitPoints + result.gain} HP`)
                ...
                className={`text-xs mt-2 ${character.level >= 20 ? 'text-gray-600 cursor-not-allowed' : 'text-dark-purple-400 hover:text-white'}`}
              >
                {character.level >= 20 ? 'Max Level' : '+ Roll Level Up HP'}
              </button>
            </div>
            ...
            <div className="bg-dark-purple-950 p-4 rounded-lg border border-dark-purple-700 text-center">
              ...
              <div className="text-3xl font-bold text-white">{character.initiative >= 0 ? '+' : ''}{character.initiative || 0}</div>
              <button onClick={() => handleRoll(rollInitiative, 'Initiative')} className="text-xs text-dark-purple-400 hover:text-white mt-1">🎲 Roll</button>
            </div>
              ...
              <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-6 shadow-xl">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select value={character.race} onChange={(e) => updateCharacter('race', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all">
                  ...
                  <div>
                    ...
                    <select value={character.alignment} onChange={(e) => updateCharacter('alignment', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all">
              ...
              <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-6 shadow-xl">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="range" min={currentRace.ageRange.min} max={currentRace.ageRange.max} value={ageValue} onChange={(e) => updateCharacter('age', parseInt(e.target.value))} className="w-full h-2 bg-dark-purple-950 rounded-lg appearance-none cursor-pointer accent-dark-purple-500" />
                  </div>
                  <div>
                    ...
                    <input type="range" min={currentRace.heightRange.min} max={currentRace.heightRange.max} value={heightValue} onChange={(e) => updateCharacter('height', parseInt(e.target.value))} className="w-full h-2 bg-dark-purple-950 rounded-lg appearance-none cursor-pointer accent-dark-purple-500" />
                  </div>
                  <div>
                    ...
                    <input type="range" min={currentRace.weightRange.min} max={currentRace.weightRange.max} value={weightValue} onChange={(e) => updateCharacter('weight', parseInt(e.target.value))} className="w-full h-2 bg-dark-purple-950 rounded-lg appearance-none cursor-pointer accent-dark-purple-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-2 text-dark-purple-300">Gender</label>
                    <input type="text" value={character.gender || ''} onChange={(e) => updateCharacter('gender', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all" placeholder="e.g., Male, Female, Non-binary" />
                  </div>
                </div>
              </div>
            </div>
            <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-6 shadow-xl">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                  ...
                  <select value={character.eyes || ''} onChange={(e) => updateCharacter('eyes', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all">
                ...
                <div>
                  ...
                  <select value={character.hair || ''} onChange={(e) => updateCharacter('hair', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all">
                    <option value="">Select...</option>
                ...
                <div>
                  ...
                  <select value={character.hairStyle || ''} onChange={(e) => updateCharacter('hairStyle', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all">
                    <option value="">Select...</option>
                ...
                <div>
                  ...
                  <select value={character.skin || ''} onChange={(e) => updateCharacter('skin', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all">
                    <option value="">Select...</option>
                ...
                <div>
                  ...
                  <select value={character.favoriteColor || ''} onChange={(e) => updateCharacter('favoriteColor', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all">
                    <option value="">Select...</option>
            ...
            <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-6 shadow-xl">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <select value={character.class} onChange={(e) => updateCharacter('class', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all">
                ...
                <div>
                  ...
                  <input type="number" min="1" max="20" value={character.level} onChange={(e) => updateCharacter('level', parseInt(e.target.value) || 1)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-2 text-dark-purple-300">Background</label>
                  <select value={character.background} onChange={(e) => updateCharacter('background', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {Object.entries(character.abilities).map(([ability, score]) => (
                  <div key={ability} className="bg-dark-purple-950 p-4 rounded-lg border border-dark-purple-700">
                    ...
                    <input type="number" min="1" max="20" value={score} onChange={(e) => updateAbility(ability, e.target.value)} className="w-full bg-dark-purple-900 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white text-center text-2xl font-bold mb-2 focus:outline-none focus:border-dark-purple-500 transition-all" />
                    <div className="text-center text-lg font-bold text-dark-purple-300">Modifier: {(score - 10) >= 0 ? '+' : ''}{Math.floor((score - 10) / 2)}</div>
                    {character.modifiers && character.modifiers[ability] !== Math.floor((score - 10) / 2) && (
                      <div className="text-center text-sm text-dark-purple-400 mt-1">With racial ASI: {character.modifiers[ability] >= 0 ? '+' : ''}{character.modifiers[ability]}</div>
                    )}
                    <button onClick={() => handleRoll(() => rollSavingThrow(ability), `${abilityLabels[ability]} Save`)} className="mt-2 w-full bg-dark-purple-800 hover:bg-dark-purple-700 text-white text-sm py-2 px-4 rounded transition-all">Roll Save</button>
                  </div>
                ))}
              </div>
            </div>
                ...
                {Object.entries(character.savingThrows || {}).map(([ability, value]) => (
                  <div key={ability} className="bg-dark-purple-950 p-3 rounded-lg border border-dark-purple-700 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleRoll(() => rollSavingThrow(ability), `${abilityLabels[ability]} Save`)} className="text-dark-purple-400 hover:text-white text-xs">🎲</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-6 shadow-xl space-y-4">
              <div>
                ...
                <textarea value={character.personalityTraits || ''} onChange={(e) => updateCharacter('personalityTraits', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all min-h-[100px]" placeholder="Describe your personality..." />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2 text-dark-purple-300">Ideals</label>
                <textarea value={character.ideals || ''} onChange={(e) => updateCharacter('ideals', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all min-h-[100px]" placeholder="What do you believe in?" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2 text-dark-purple-300">Bonds</label>
                <textarea value={character.bonds || ''} onChange={(e) => updateCharacter('bonds', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all min-h-[100px]" placeholder="What ties you to the world?" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2 text-dark-purple-300">Flaws</label>
                <textarea value={character.flaws || ''} onChange={(e) => updateCharacter('flaws', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all min-h-[100px]" placeholder="What are your weaknesses?" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-2 text-dark-purple-300">Backstory</label>
                <textarea value={character.backstory || ''} onChange={(e) => updateCharacter('backstory', e.target.value)} className="w-full bg-dark-purple-950 border-2 border-dark-purple-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-dark-purple-500 transition-all min-h-[200px]" placeholder="Tell your story..." />
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-dark-purple-900/50 backdrop-blur-sm border border-dark-purple-700 rounded-xl p-6 shadow-xl text-center">
              ...
              <div className="grid md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                <button className="bg-dark-purple-800 hover:bg-dark-purple-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200 border border-dark-purple-600">Create Custom Race</button>
                <button className="bg-dark-purple-800 hover:bg-dark-purple-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200 border border-dark-purple-600">Create Custom Class</button>
                <button className="bg-dark-purple-800 hover:bg-dark-purple-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200 border border-dark-purple-600">Create Custom Background</button>
                <button className="bg-dark-purple-800 hover:bg-dark-purple-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200 border border-dark-purple-600">Create Custom Item</button>
              </div>
            </div>
          </div>
        )}
