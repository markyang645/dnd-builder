...
export default function ExportTab() {
  ...
  return (
    <div className="p-6 bg-tab-orange/20 backdrop-blur-sm rounded-xl m-4">
      ...
      <button onClick={handleCopy} className="btn-primary mb-4" disabled={!character}>Copy JSON to Clipboard</button>
      <pre className="bg-dark-purple-950/80 border border-orange-600/50 text-gray-100 p-4 rounded-lg text-xs overflow-auto max-h-96">
        {character ? JSON.stringify(character, null, 2) : '// Enter character details to see export data'}
      </pre>
    </div>
  );
}